...
public class TeleporterObject : MonoBehaviour {
	...
	void OnTriggerEnter(Collider other)
    {
		...
		if(playerController != null && playerController.isActiveAndEnabled)
        {
			...
			if (td == null)
            {
				Debug.LogErrorFormat("TeleporterOnject: Character [{0}] Enter Teleporter [{1}], But TeleporterDefine not existed", playerController.character.Info.Name, this.ID);
				return;
            }
			Log.InfoFormat("TeleporterObject: Character [{0}] Enter Teleporter [{1}]:[{2}]", playerController.character.Info.Name, td.ID, td.Name);
			if(td.LinkTo > 0)
            {
